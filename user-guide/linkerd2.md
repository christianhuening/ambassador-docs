...
3. Deploy Ambassador. Note: If this is your first time deploying Ambassador, reviewing the [Ambassador quick start](/user-guide/getting-started) is strongly recommended.
...
2. Deploy the QOTM demo application. The QOTM application
    ...
    Save the above to a file called `qotm.yaml` and run `kubectl apply -f qotm.yaml`. Watch via `kubectl get po` as the Pod is created. Note that it starts with `0/2` containers automatically, as it has been auto-injected by the Linkerd2 Webhook.
...
4. Create a `Mapping` for the `qotm-Linkerd2` service.
   ...
   spec:
     ...
     - name: http
       port: 80
   ```
Save the above YAML to a file named `qotm-mapping.yaml`, and use `kubectl apply -f qotm-mapping.yaml` to apply this configuration to your Kubernetes cluster. Note that in the above config there is nothing special to make it work with Linkerd2. The general config for Ambassador already adds Linkerd Headers when forwarding requests to the service mesh.
...
If you now add TLS to your Ambassador endpoint, you have an end-to-end secured connection.
